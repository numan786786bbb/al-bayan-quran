...
const QuranMushafView: React.FC<QuranMushafViewProps> = ({ initialPage = 1 }) => {
  const [currentPage, setCurrentPage] = useState(initialPage);
  const [isLoading, setIsLoading] = useState(false);

  const totalPages = 604; // Total pages in the Mushaf
  ...
  const handlePageChange = (newPage: number) => {
    ...
    return require(`../../assets/quran/page_${page.toString().padStart(3, '0')}.webp`);
  };
