...
import muhammadNamesData, { MuhammadName } from '../utils/muhammadNamesData';
import MuhammadNamesList from '../components/MuhammadNamesList';
import { useSettings } from '../context/SettingsContext';

export default function MuhammadNamesScreen() {
  const navigation = useNavigation();
  const { settings } = useSettings();
  const [currentTime, setCurrentTime] = useState(new Date());
  const [searchQuery, setSearchQuery] = useState('');
  const [filteredNames, setFilteredNames] = useState<MuhammadName[]>(muhammadNamesData);
  const [favorites, setFavorites] = useState<string[]>([]);
  const [showFavorites, setShowFavorites] = useState(false);

  // Track screen view
  useEffect(() => {
    analytics.trackScreenView('Muhammad Names');
    analytics.trackFeatureUsage('Asma-un-Nabi Browser');
  }, []);
  ...
  useEffect(() => {
    let filtered = muhammadNamesData;
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.titleContainer}>
          <MaterialCommunityIcons name="star-crescent" size={24} color="white" />
          <Text style={styles.screenTitle}>Asma-un-Nabi</Text>
        </View>
      ...
      <View style={styles.subtitleContainer}>
        <Text style={styles.subtitle}>The Beautiful Names of Prophet Muhammad ﷺ</Text>
      </View>
      ...
      <View style={styles.contentContainer}>
        {filteredNames.length > 0 ? (
          <MuhammadNamesList 
            names={filteredNames} 
            onSelectName={(name) => {
              navigation.navigate('MuhammadNameDetail' as never, { name } as never);
